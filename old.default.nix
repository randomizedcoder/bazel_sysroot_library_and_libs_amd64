...
let
  ...
  commonLibs = with pkgs; [
    ...
    # Core system libraries - Provides startup files (crt1.o, crti.o, etc.)
  ...
  # Create a derivation that copies the sysroot files
  sysroot = pkgs.stdenv.mkDerivation {
    ...
    # Copy all .o files from glibc
    postInstall = ''
      mkdir -p $out/sysroot/lib
      cp --dereference --recursive ${pkgs.glibc}/lib/*.o $out/sysroot/lib/
    '';
